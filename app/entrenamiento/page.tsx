...
async function cerrarSesion() {
  ...
  const Anatomia = ({ musculo }: { musculo: string }) => {
    ...
    const zonas: Array<{cx:number;cy:number;rx:number;ry:number}> = [];
    if (m.includes("pecho")) zonas.push({cx:50,cy:35,rx:17,ry:8});
    if (m.includes("hombro") || m.includes("deltoide")) zonas.push({cx:29,cy:34,rx:7,ry:7},{cx:71,cy:34,rx:7,ry:7});
    if (m.includes("bíceps") || m.includes("biceps") || m.includes("braquial")) zonas.push({cx:24,cy:50,rx:5,ry:10},{cx:76,cy:50,rx:5,ry:10});
    if (m.includes("tríceps") || m.includes("triceps")) zonas.push({cx:23,cy:48,rx:5,ry:11},{cx:77,cy:48,rx:5,ry:11});
    if (m.includes("dorsal") || m.includes("espalda") || m.includes("trapecio") || m.includes("lumbar")) zonas.push({cx:50,cy:43,rx:18,ry:14});
    if (m.includes("abdomen") || m.includes("core") || m.includes("oblic")) zonas.push({cx:50,cy:57,rx:12,ry:14});
    if (m.includes("cuádriceps") || m.includes("cuadriceps") || m.includes("aductor")) zonas.push({cx:40,cy:79,rx:7,ry:16},{cx:60,cy:79,rx:7,ry:16});
    if (m.includes("femoral") || m.includes("glúteo") || m.includes("gluteo")) zonas.push({cx:40,cy:76,rx:7,ry:15},{cx:60,cy:76,rx:7,ry:15});
    if (m.includes("gemelo") || m.includes("sóleo") || m.includes("soleo")) zonas.push({cx:39,cy:105,rx:6,ry:13},{cx:61,cy:105,rx:6,ry:13});
    if (m.includes("antebrazo") || m.includes("agarre")) zonas.push({cx:18,cy:65,rx:4,ry:11},{cx:82,cy:65,rx:4,ry:11});
    return <div className="vf-anatomy" title={`Zona principal: ${musculo}`}><svg viewBox="0 0 100 125" aria-label={`Músculos trabajados: ${musculo}`}><circle cx="50" cy="12" r="9" className="body"/><path d="M38 24 Q50 20 62 24 L68 60 Q62 70 60 72 L66 118 L55 118 L50 78 L45 118 L34 118 L40 72 Q32 65 32 60 Z" className="body"/><path d="M35 29 L20 62 M65 29 L80 62" className="limb"/>{zonas.map((z,i)=><ellipse key={i} {...z} className="muscle"/>)}</svg><span>{musculo}</span></div>;
  };
    ...
    <main className="vf-app">
      <style>{`
        *{box-sizing:border-box} body{margin:0;background:#020402} button,input,select{font:inherit}
        ...
        .vf-anatomy{width:82px;min-width:82px;text-align:center;color:#8DFF00;font-size:8px;font-weight:900;text-transform:uppercase}.vf-anatomy svg{display:block;width:64px;height:78px;margin:auto;filter:drop-shadow(0 0 7px rgba(141,255,0,.12))}.vf-anatomy .body{fill:#171c17;stroke:#7b857b;stroke-width:2}.vf-anatomy .limb{fill:none;stroke:#7b857b;stroke-width:7;stroke-linecap:round}.vf-anatomy .muscle{fill:#ff2d2d;stroke:#ff6b6b;stroke-width:1;filter:drop-shadow(0 0 3px rgba(255,45,45,.75))}.vf-pause{margin-top:9px;border:1px solid #8DFF00;background:#0a1107;color:#8DFF00;border-radius:8px;padding:6px 10px;font-size:10px;font-weight:1000;cursor:pointer}.vf-pause.paused{background:#8DFF00;color:#050805}.vf-lib-top{display:grid;grid-template-columns:1fr auto;gap:8px;align-items:center}.vf-lib-muscle{color:#ff3b3b;font-size:10px;font-weight:1000;text-transform:uppercase;margin-top:4px}
